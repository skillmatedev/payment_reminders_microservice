...
import WorkshopBooking from "../models/WorkshopBooking";
...
export async function runReminderJob() {
  ...
  const liveWorkshops = await Workshop.find({ status: "live" }).select("_id");
  const workshopIds = liveWorkshops.map((w) => w._id);
